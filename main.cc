	...
	if (choice == 1) {
		...
		while (true) {
			...
			if (choice == 1) { //Add Manpower
				...
				cout << pti.delete_amount(soldiers, soldiers) << endl;
			}
			else if (choice == 6) {
				...
				bool deleted = false;
				while (true) {
					//Write code here to keep deleting provinces from pta until we fail to delete with our remaining diplo points
					string s = pta.delete_amount(points, points);
